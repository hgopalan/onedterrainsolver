...
class powerForecast:
    ...
    def get_HRRR(self):
        ...
        self._get_desired_hrrr_variables()
        # First get the current non-forecast time
        H = self._get_single_HRRR(curr_fxx = 0)
        # U Values 
        ux=H[":UGRD:80 m above ground"]
        # V Values 
        vx=H[":VGRD:80 m above ground"]
        # Theta 2 m 
        airtemp=H[":POT:2 m above ground"]
        # Planetary Boundary Layer 
        pblh=H[":HPBL:surface"]
        # Sensible heat flux 
        sflx=H[":SHTFL:surface"]
        # Roughness length 
        z0=H[":SFCR:surface"]
        # Terrain Height 
        terrain_ht=H[":HGT:surface"]
        # Now, loop over the requested forecast, if any
        # for curr_fxx in self.desired_fxx_hours:
        #     H = self._get_single_HRRR(curr_fxx)
        return(ux.values[0][0],vx.values[0][0],airtemp.values[0][0],pblh.values[0][0],sflx.values[0][0]/1000.0,z0.values[0][0],terrain_ht.values[0][0])
    ...
    def _get_single_HRRR(self, curr_fxx):
        ...
        H = Herbie(datetime_oi_hrrr, model=self.model, product=self.product, fxx=int(curr_fxx), verbose=False)
        ...
        for var in self.desired_hrrr_vars:
            ...
            dsi = ds.herbie.pick_points(points) #herbie.nearest_points(self.loc)
...
if __name__ == "__main__":
    from hrrr_1dsolver_terrain import generate_profile
    ...
    minute=0
    second=0
    for time_value in range(10,16):
        # --------------- USER INPUT ----------------
        loc = (-105, 39)
        #datetime = "2024-09-15 14:00:00"
        datetime_object=start_datetime = datetime(year,month,day,time_value,minute,second)
        fxx_period = '0'
        avgAroundLoc = False
        # ---------- END OF USER INPUT --------------
        pf = powerForecast(loc, datetime_oi_local=datetime_object,fxx_period=fxx_period)
        ux,vx,airtemp,pblh,sflx,z0,terrain_ht=pf.get_HRRR()
        print(ux,vx,airtemp,pblh,sflx,z0,terrain_ht)
        # Let us predict 
        start=time.time()
        allowed_error=0.25
        metMastHeight=80+terrain_ht
        metMastWind=[ux,vx]
        dz=16
        zheight=terrain_ht+2048
        npts=int(zheight/dz)
        roughness_length=z0
        terrain_ht=terrain_ht
        coriolis=loc[1]
        inv_height=pblh+terrain_ht
        inv_width=0
        inv_strength=8
        lapse_rate=0.003
        heat_flux_mode=1
        heat_flux_mode_value=sflx
        num_of_steps=50000
        tolerance=1e-2
        if(time_value==10):
            initial_ug=2*ux
